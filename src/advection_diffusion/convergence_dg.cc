...
solve_for_element_order(const int element_order, int max_refinement,
    ...
    std::ofstream file("errors-o" + std::to_string(element_order)
                       + "-eps=" + std::to_string(eps) + "-dg.csv");
    AdvectionDiffusionDG<dim>::write_header_to_file(file);
    ...
    for (int n_refines = 2; n_refines < max_refinement + 1; ++n_refines) {
        ...
        AdvectionDiffusionDG<dim> fem(element_order, n_refines, eps,
                                    rhs, bdd_values, analytical_solution);
        Error error = fem.run();
        std::cout << "|| u - u_h ||_L2 = " << error.l2_error << std::endl;
        AdvectionDiffusionDG<dim>::write_error_to_file(error, file);
    }
}
...
int main() {
    double eps = 0.1;
    run_convergence_test<2>({1, 2}, 6, eps);
